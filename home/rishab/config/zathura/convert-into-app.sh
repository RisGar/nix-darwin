...
BASE_PATH=$(nix eval /private/etc/nix-darwin#darwinConfigurations."Rishabs-MacBook-Pro".pkgs.zathura.outPath --raw)

ZATHURA_EXE_DEFAULT="$BASE_PATH/bin/zathura"
...
find_plugin() {
  plugin_prefix="$BASE_PATH/lib/zathura"
...
if [[ -z ${CB_PLUGIN}${DJVU_PLUGIN}${MUPDF_PLUGIN}${POPPLER_PLUGIN}${PS_PLUGIN} ]]; then
...
if [[ -n ${MUPDF_PLUGIN} && -n ${POPPLER_PLUGIN} ]]; then
...
/usr/bin/curl -o "${ZATHURA_APP}/Contents/Resources/AppIcon.icns" "${ZATHURA_ICON_URL}"
